...
function updateBooking() {
    ...
    let invalidProvisionInput = false;
    ...
    selectedProvisionIds.forEach(provisionId => {
        ...
        // Check if remainingExecutions is not provided or is less than or equal to 0
        if (isNaN(remainingExecutions) || remainingExecutions < 0) {
            if (!alertDisplayedProvision) {
                ...
                alertElement.textContent = "The number of remaining executions must be greater than or equal to zero";
                parentElement.insertBefore(alertElement, parentElement.firstChild);
                window.scrollTo({ top: 0, behavior: 'smooth' });
                alertDisplayedProvision = true;
            }

            // Set the flag to true if an invalid provision input is found
            invalidProvisionInput = true;
        } else {
    ...
    // If there is any invalid provision input, stop further execution
    if (invalidProvisionInput) {
        return;
    }
